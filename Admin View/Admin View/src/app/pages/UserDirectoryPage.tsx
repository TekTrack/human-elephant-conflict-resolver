...
export function UserDirectoryPage() {
  ...
  const fetchAllData = async () => {
    try {
      ...
      const [userRes, adminRes] = await Promise.all([
        ...
        axios.get(`${API_BASE_URL}/api/admin/alladmins`, config),
      ]);
  ...
  const handleCreateUserandAdmin = async () => {
    ...
    try {
      ...
      if (userType === "Admin") {
        endpoint = `${API_BASE_URL}/api/admin/newadmin`;
        payload = {
        ...
        endpoint = `${API_BASE_URL}/api/admin/createuser`;
        payload = {
          name: formData.name,
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      ...
      if (deleteTarget.role === "Admin") {
        endpoint = `${API_BASE_URL}/api/admin/deleteadmin`;
        payload = { adminId: deleteTarget.adminId };
      } else {
        endpoint = `${API_BASE_URL}/api/admin/deleteuser`;
  ...
  return (
    ...
    {viewUser && (
      <PopupCard
        ...
        footer={
          <Button variant="ghost" className="w-full" onClick={() => setViewUser(null)}>
            Close
          </Button>
        }
      >
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <Card className={`p-5 border ${isDark ? "border-white/10" : "border-gray-200"}`}>
          ...
          <p className={`text-3xl font-bold mb-3 ${isDark ? "text-blue-500" : "text-purple-600"}`}>
        ...
        <>
          {loading ? (
            ...
            <div className="p-20 text-center flex flex-col items-center gap-4 text-red-500">
              ...
              <Button onClick={fetchAllData} variant="ghost" className="mt-2 group">

...
const fetchDrones = async (): Promise<Drone[]> => {
  try {
    ...
    if (!res.ok) {
      console.error("Failed to fetch drones data from server");
      return [];
    }
    ...
    console.error("Network Error: Please check your connection and try again.", err);
    return [];
  }
};
...
export function DroneMapPage() {
  ...
  useEffect(() => {
    const loadDrones = async () => {
      ...
      try {
        ...
        if (data.length === 0) {
          setError("No drones found or failed to fetch.");
        }
        setDrones(data);
      } catch (err) {
  ...
  const savedPathsGeoJSON = useMemo(() => {
    return {
      type: "FeatureCollection" as const,
      features: drones
          .filter((d) => d.path.length > 1 && d.id !== selectedDroneId)
          .map((d) => ({
            type: "Feature" as const,
            properties: { color: d.color },
            geometry: {
      ...
      <div className="p-8 space-y-6 flex flex-col h-full">
        ...
        <div className="grid grid-cols-1 xl:grid-cols-4 gap-6 flex-1 min-h-[600px]">
          ...
          <div className="space-y-4 xl:col-span-1 overflow-y-auto pr-2 custom-scrollbar">
            ...
            {/* Loading State */}
            {loading && (
              <p className="text-sm text-gray-400 text-center py-4">Loading drones...</p>
            )}
